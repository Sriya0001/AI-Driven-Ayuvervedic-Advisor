...
async def on_message(message: cl.Message):
    ...
    # Optimization: Only keep the last 2 rounds of conversation in memory to save tokens
    history_str = ""
    for msg in message_history[-4:-1]: # Last 4 items, excluding the current message we just added
        role = "Human" if msg["role"] == "user" else "Assistant"
        history_str += f"{role}: {msg['content'][:200]}...\n" # Truncate history to save tokens

    try:
        # Search the PDFs using ONLY the current question for better accuracy
        # But give the LLM a snippet of the history for context
        res = await chain.ainvoke(f"History: {history_str}\nQuestion: {message.content}", callbacks=[cb])
        answer = res["result"]
        print(answer)
        ...
        error_msg = f"Oops! The chatbot encountered an error (often this is a Groq Rate Limit if we're sending too much text too fast). Details: {str(e)}"
